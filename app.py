...
# ATENÇÃO: Os nomes dos arquivos (file) devem ser EXATOS como aparecem no HF
TEXT_MODELS = {
"gemma-1b": {
...
# MODELOS UNCENSORED CORRIGIDOS
"qwen2.5-14b-uncensored": {
"backend": "gguf",
"repo": "bartowski/Qwen2.5-14B-Instruct-GGUF", # Usa o base, mas com prompt engineering remove censura, OU use o fork específico se existir.
# Correção: Não existe "Qwen Uncensored" oficial do bartowski. Vou usar o Dolphin-Qwen que é uncensored.
...
"backend": "gguf",
"repo": "bartowski/Nous-Hermes-3-Llama-3.2-11B-GGUF", # Corrigido repo
"file": "Nous-Hermes-3-Llama-3.2-11B-Q4_K_M.gguf", # Corrigido file (verifique maiúsculas)
"label": "Nous-Hermes-3 11B",
"desc": "🖊️ Roleplay/escrita criativa • ~7.2GB GGUF",
},
"dolphin-3.0-8b-uncensored": {
"backend": "gguf",
"repo": "cognitivecomputations/Dolphin-3.0-Llama-3.1-8B-GGUF", # Corrigido repo
"file": "Dolphin-3.0-Llama-3.1-8B-Q4_K_M.gguf", # Corrigido file
"label": "Dolphin 3.0 Llama3.1 8B 🐬",
"desc": "🥧 Geral uncensored • código+math+NSFW • ~5.4GB GGUF",
},
"llama3-2-3b-uncensored": {
"backend": "gguf",
"repo": "bartowski/Llama-3.2-3B-Instruct-GGUF", # O bartowski não tem "Uncensored" no nome, mas o base é ajustável.
# Melhor opção uncensored pequena: Sao10K/L3-8B-Stheno-v3.2 é 8B. Para 3B, use o Tulu ou similar.
# Vou usar o Llama 3.2 3B base e aplicar system prompt agressivo, ou usar um fork.
# Fork confiável: "bartowski/Llama-3.2-3B-Instruct-GGUF" (não é uncensored nativo, mas leve).
# Para UNcensored 3B real: "bartowski/TinyLlama-1.1B-Chat-v1.0-GGUF" é muito fraco.
# Vou trocar por Stheno 8B que é o rei do uncensored médio.
...
"backend": "gguf",
"repo": "bartowski/DeepSeek-R1-Distill-Qwen-14B-GGUF", # Oficial Distill
...
print(f" Descarregando modelo anterior ({_current['key']})...")
_current["key"] = None
...
model_id = cfg["id"]
print(f" Carregando modelo (transformers): {model_id}...")
...
repo = cfg['repo']
filename = cfg['file']
print(f" Carregando GGUF: {repo}/{filename}...")

# Tenta baixar/carregar
...
print(f" Modelo {key} carregado!")
return _current

# Pré-carrega o modelo padrão no boot
print(f" Pré-carregando modelo padrão ({DEFAULT_MODEL_KEY})...")
...
print(f" Carregando modelo de imagem {IMAGE_MODEL_ID}...")
...
print(f" Resposta gerada em {elapsed:.1f}s ({backend})")
